...
def record_performances(classifier, prediction, duration, Y_test):
    ...
    resultnp = (numpy.array(results[0]))
    if rank > 0:
        comm.Send(resultnp, dest=0)
...
def logisticregression(X_train, X_test, Y_train, Y_test):
    ...
    t = end - start
    record_performances("Logistic Regression", y_pred_lr, t, Y_test)
...
def perceptron(X_train, X_test, Y_train, Y_test):
    ...
    t = end - start
    record_performances("Perceptron", y_pred_ppn, t, Y_test)
...
def svmlinear(X_train, X_test, Y_train, Y_test):
    ...
    t = end - start
    record_performances("Support Vector Machines", y_pred_svm, t, Y_test)
...
def decisiontree(X_train, X_test, Y_train, Y_test):
    ...
    t = end - start
    record_performances("Decision Tree", y_pred_dtree, t, Y_test)
...
def randomforest(X_train, X_test, Y_train, Y_test):
    ...
    t = end - start
    record_performances("Random Forest Classifier", y_pred_rfc, t, Y_test)


comm.Barrier()
fn_list = [logisticregression, perceptron, svmlinear, decisiontree, randomforest]
if np >= len(fn_list):
    for i in range(0, len(fn_list)):
        if rank == i:
            print(i)
            fn_list[i](X_train, X_test, Y_train, Y_test)
else:
    for i in range(0, len(fn_list)):
        if rank == (i % np):
            fn_list[i](X_train, X_test, Y_train, Y_test)
...
for i in range(1, len(fn_list)):
    c = comm.Recv(resultnp)

overall_end = timer()
overall_time = overall_end - overall_start

if rank >= 0 and rank < len(fn_list):
