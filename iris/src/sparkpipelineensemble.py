...
y_pred_lr = lr_model.transform(test_df_lr)
end = timer()
t = end - start
...
train_df_ppn, test_df_ppn = train_df, test_df
start = timer()
#ppn = Perceptron(eta0=0.1, random_state=1)
ppn = MLP(featuresCol="X", labelCol="Y", layers=[4, 3], maxIter=100, blockSize=128)
ppn_pipeline = Pipeline(stages=[ppn])
ppn_model = ppn_pipeline.fit(train_df_ppn)
y_pred_ppn = ppn_model.transform(test_df_ppn)
end = timer()
t = end - start
record_performances("Perceptron", y_pred_ppn, t)


# Currently there is no support for LinearSVC multiclass classifier. So this step is ignored.
"""
from pyspark.ml.classification import LinearSVC
train_df_svm, test_df_svm = train_df, test_df
start = timer()
svm_clf = LinearSVC(maxIter=1000, regParam=0.1, featuresCol="X", labelCol="Y")
svc_pipeline = Pipeline(stages=[svm_clf])
svc_pipeline.fit(train_df_svm)
y_pred_svm = svm_clf.transform(test_df_svm)
end = timer()
t = end - start
...
clf_pipeline = Pipeline(stages=[clf])
clf_pipeline = clf_pipeline.fit(train_df_clf)
y_pred_dtree = clf_pipeline.transform(test_df_clf)
end = timer()
t = end - start
...
rfc_pipeline = rfc_pipeline.fit(train_df_rfc)
y_pred_rfc = rfc_pipeline.transform(test_df_rfc)
end = timer()
t = end - start
